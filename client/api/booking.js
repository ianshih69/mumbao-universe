...
async function handleRequest(req, res, requestId) {
  ...
  if (!body || typeof body !== "object" || Array.isArray(body)) {
    throw httpError(400, "請提供預約資料。", "invalid_payload");
  }

  const { checkIn, checkOut } = validateDateRange(body.check_in, body.check_out);
  const guestName = cleanText(body.guest_name, 80);
  const guestEmail = cleanText(body.guest_email, 160).toLowerCase();
  const guestPhone = cleanText(body.guest_phone, 60);
  const notes = cleanText(body.notes, 1000);
  const guestCount = body.guest_count === "" || body.guest_count === undefined ? null : Number(body.guest_count);

  if (!guestName) throw httpError(400, "請填寫姓名。", "missing_guest_name");
  if (!guestEmail && !guestPhone) {
    throw httpError(400, "請至少留下 Email 或電話，方便我們回覆。", "missing_contact");
  }
  if (guestEmail && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(guestEmail)) {
    throw httpError(400, "Email 格式不正確。", "invalid_email");
  }
  if (guestCount !== null && (!Number.isInteger(guestCount) || guestCount < 1 || guestCount > 30)) {
  ...
  await supabaseRequest("/booking_availability_alerts", {
    ...
    body: JSON.stringify({
      ...
      title: "官網預約申請待確認",
      description: `${guestName} 申請 ${checkIn} 至 ${checkOut} 入住。`,
      check_in: checkIn,
      check_out: checkOut,
      source: "website",
...
export default async function handler(req, res) {
  ...
  try {
    ...
    sendJson(res, status, {
      ...
      message: status >= 500 ? "線上預約暫時無法送出，請稍後再試。" : error.message,
    });
  }
}
