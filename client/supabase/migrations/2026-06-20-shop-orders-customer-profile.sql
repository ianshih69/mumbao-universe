...
create index if not exists shop_orders_customer_profile_id_created_at_idx
  on public.shop_orders(customer_profile_id, created_at desc);
...
declare
  ...
  raw_customer_profile_id text := nullif(trim(coalesce(order_payload->>'customer_profile_id', '')), '');
  next_customer_profile_id uuid := null;
  ...
  inv_after integer;
begin
  if raw_customer_profile_id is not null then
    next_customer_profile_id := raw_customer_profile_id::uuid;
  end if;
  ...
  return jsonb_build_object(
    ...
    'customer_profile_id', next_customer_profile_id
  );
end;
$$;
