...
create index if not exists shop_inventory_movements_reference_id_idx
  on public.shop_inventory_movements(reference_id);

create unique index if not exists shop_inventory_movements_order_cancel_restock_unique_idx
  on public.shop_inventory_movements(reference_type, reference_number, variant_id)
  where reference_type = 'order_cancel_restock';

create or replace function public.adjust_shop_inventory(adjustment_payload jsonb)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  next_reference_id := nullif(trim(coalesce(adjustment_payload->>'reference_id', '')), '')::uuid;
...
revoke all on function public.adjust_shop_inventory(jsonb) from public;
revoke all on function public.adjust_shop_inventory(jsonb) from anon;
revoke all on function public.adjust_shop_inventory(jsonb) from authenticated;
grant execute on function public.adjust_shop_inventory(jsonb) to service_role;
